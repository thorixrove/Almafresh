...
export default function InsightsStatsSection() {
  ...
  const pendingItems = totalItem - completedItems

  const completionRate = totalItem ? Math.round((completedItems / totalItem) * 100) : 0
  ...
  return(
    ...
    <View className="flex-row gap-2">
      ...
      <View className="flex-1 rounded-3xl border border-border bg-card p-4">
        ...
        <Text className="mt-1 text-3xl font-extrabold text-foreground">{totalItem}</Text>
      </View>
    </View>
